...
echo *******************************************
echo * Microservices Startup Script with Profiles *
echo *******************************************
...
start "Eureka Server" cmd /c "mvn spring-boot:run -P eureka >> logs\eureka-server.log 2>&1"
...
start "User Service" cmd /c "mvn spring-boot:run -P user >> logs\user-service.log 2>&1"
...
start "File Service" cmd /c "mvn spring-boot:run -P file >> logs\file-service.log 2>&1"
...
start "Repository Service" cmd /c "mvn spring-boot:run -P repository >> logs\repository-service.log 2>&1"
...
start "API Gateway" cmd /c "mvn spring-boot:run -P gateway >> logs\api-gateway.log 2>&1"
